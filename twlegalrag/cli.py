"""Taiwan Legal RAG CLI — retrieve Taiwan court judgments for use with your own AI.
    ...
    twlegalrag health
    twlegalrag ask "..."                            # optional: BYO-LLM draft (unverified)

This tool retrieves judgments and packages them. It does not generate legal
advice by default and does not guarantee the faithfulness of any model output.
"""
...
from .config import get_tlr_api_key, get_tlr_base_url, load_llm_config
from .llm import LLMError, build_messages, stream_answer
...
def search(
    ...
    read: bool = typer.Option(False, "--read", help="同時抓取判決理由全文"),
) -> None:
...
def _print_report(rep: VerifyReport) -> None:
    ...
    if rep.verdicts:
        ...
        console.print(table)
    fq = rep.fake_quote
    if fq.get("status") != "pass":
        s = _STATUS_STYLE.get(fq["status"], "white")
        console.print(f"逐字引文檢查 (窄): [{s}]{_STATUS_LABEL.get(fq['status'])}[/] ({fq['reason']})")
    if rep.out_of_bundle:
        console.print(
            "[bold red]⚠ 偵測到引用了不在 bundle 內的判決字號 — 高度疑似捏造,請勿直接採信。[/]"
        )
    console.print(
        "[dim]註: 此檢查僅驗「引用字號是否在 bundle 內 + 窄義逐字引文」,"
        "不保證見解讀對、不檢查當事人主張被當成法院見解。請自行核對判決全文。[/]"
    )
...
@app.command()
def ask(
    question: str = typer.Argument(..., help="法律問題 (白話即可)"),
    n: int = typer.Option(5, "--n", "-n", help="檢索判決筆數 (1-10)"),
    read_top: Optional[int] = typer.Option(
        None, "--read-top", help="抓全文的前 N 筆 (預設全部)"
    ),
    no_check: bool = typer.Option(False, "--no-check", help="跳過引用檢查"),
) -> None:
    """[BYO LLM, unverified draft] Retrieve, then draft an answer with YOUR LLM.

    Optional convenience command. The draft is generated by your own LLM account
    and is NOT verified or endorsed by this tool or its backend. For most uses
    prefer `pack` (hand the bundle to your own AI) — it keeps you in control of
    which model answers and how. A citation check runs at the end but only covers
    citation identity, not legal reasoning.
    """
    cfg = load_llm_config()
    try:
        with _client() as c:
        ...
        raise typer.Exit(1)
    if not hits:
        err.print("[yellow]檢索無結果,無法作答。[/]")
        raise typer.Exit(1)
    err.print(
        f"[yellow]⚠ 以下草稿由您自己的 LLM ({cfg.provider}:{cfg.model}) 生成,"
        f"未經本工具或後端驗證或背書。[/]"
    )
    err.print(
        f"[dim]檢索到 {len(hits)} 篇判決,"
        f"{sum(1 for j in hits if j.fulltext)} 篇取得全文。送交您的 LLM 分析...[/]\n"
    )
    messages = build_messages(question, hits)
    answer_parts: list[str] = []
    try:
        for tok in stream_answer(cfg, messages):
    ...
    if no_check:
        return
    rep = citation_check("".join(answer_parts), hits)
    _print_report(rep)
