...
| 欄位 | 型別 | 必填 | 說明 |
|---|---|---|---|
| `query` | string | 是 | 繁體中文搜尋字串。可填法律議題、關鍵字、案號、法院名稱或法條。 |
...
- 呼叫 `get_judgment_fulltext` 時必須帶上 `result_token`(由 search 回應提供)。
- 摘錄是供引用識別用,不是完整閱讀內容。
...
| `service_unavailable` | 服務暫時無法使用(維護或限流),請稍後重試。 |
...
| `token_expired` | `result_token` 已失效,請重新搜尋。 |
| `service_unavailable` | 服務暫時無法使用(維護或限流),請稍後重試。 |
| `internal_error` | 後端錯誤,請稍後重試。 |
